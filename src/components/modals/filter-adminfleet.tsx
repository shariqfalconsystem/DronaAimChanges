...
export const FilterFleetListDialog = ({
  ...
  // Initialize date pickers from existing filter criteria or null
  const [effectiveFrom, setEffectiveFrom] = useState<Dayjs | null>(
    filterCriteria?.policyEffectiveDate?.from ? dayjs(filterCriteria.policyEffectiveDate.from) : null
  );
  const [effectiveTo, setEffectiveTo] = useState<Dayjs | null>(
    filterCriteria?.policyEffectiveDate?.to ? dayjs(filterCriteria.policyEffectiveDate.to) : null
  );
  const [expiryFrom, setExpiryFrom] = useState<Dayjs | null>(
    filterCriteria?.policyExpiryDate?.from ? dayjs(filterCriteria.policyExpiryDate.from) : null
  );
  const [expiryTo, setExpiryTo] = useState<Dayjs | null>(
    filterCriteria?.policyExpiryDate?.to ? dayjs(filterCriteria.policyExpiryDate.to) : null
  );

  // Initialize checkbox states for policy and contract status
  const [policyStatus, setPolicyStatus] = useState<Record<string, boolean>>({
    active: filterCriteria?.policyStatus?.active || false,
    expired: filterCriteria?.policyStatus?.expired || false,
  });
  ...
  const handleApplyFilter = () => {
    ...
    // Add policy status checkboxes if any are checked
          ...
          <Box display="flex" gap={2} mb={2}>
            ...
            <Box>
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    checked={policyStatus.expired}
                    onChange={() => handlePolicyStatusChange('expired')}
