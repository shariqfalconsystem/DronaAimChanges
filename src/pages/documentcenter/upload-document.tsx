...
const UploadDocumentModal: React.FC<UploadDocumentModalProps> = ({
  ...
  const [uploadedFile, setUploadedFile] = useState<File | null>(null); // Changed to single file
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      // Always take the first file and replace any existing file
    ...
    try {
      if (uploadedFile) {
        const renamedFile = new File(
          [uploadedFile],
          documentName?.trim() ? `${documentName.trim()}.${uploadedFile.name.split('.').pop()}` : uploadedFile.name,
  ...
  const isSaveDisabled = !documentLink && !uploadedFile;
