...
const SupportEmailModal = ({ open, onClose, fleetManagerId, fleetManagerName, fleetManagerEmail }: any) => {
...
[Contact Information]`;
  ...
  const initialFormData = {
    issueDescription: '',
    name: '',
    subject: `Support Request from ${fleetManagerName} [${fleetManagerId}]`,
    body: getDefaultEmailBody(fleetManagerId, fleetManagerName),
    ccEmails: [],
  };
  ...
  useEffect(() => {
    if (open) {
      setFormData((prev: any) => ({
        ...
        subject: `Support Request from ${fleetManagerName} [${fleetManagerId}]`,
        body: getDefaultEmailBody(fleetManagerId, fleetManagerName),
        ccEmails: fleetManagerEmail
          ? [{ value: fleetManagerEmail, showError: false }]
          : [{ value: '', showError: false }],
      }));
    }
  }, [open, fleetManagerEmail, fleetManagerName, fleetManagerId]);
  ...
  const handleCcEmailChange = (index: number, value: string) => {
    ...
    const isInvalidEmail = value.includes('@') && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
  ...
  const isEmailValid = (email: string) => /^[^\s@]+@[a-zA-Z0-9-]+\.[a-zA-Z]{2,}$/.test(email);
