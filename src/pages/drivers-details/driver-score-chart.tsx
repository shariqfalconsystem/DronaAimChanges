...
import { getDriverScore } from '../../services/fleetManager/driverServices';
...
const VehicleScoreChart = ({ driverId, driverLonestarId }: any) => {
  ...
  const fetchData = async (fromDate: number, toDate: number) => {
    try {
      ...
      const { data } = await getDriverScore(driverLonestarId, driverId, fromDate, toDate);
      ...
      availableData.sort((a: any, b: any) => b.driverScoreTs - a.driverScoreTs);

      const lastAvailable = dayjs(availableData[0].driverScoreTs).endOf('day');
      const firstAvailable = dayjs(availableData[availableData.length - 1].driverScoreTs).startOf('day');
      ...
      availableData.forEach((item: any) => {
        const formattedDate = dayjs(item.driverScoreTs).format('YYYY-MM-DD');
            ...
            {processedData.length > 0 ? (
              <LineChart data={processedData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                ...
                <Line
                  ...
                  activeDot={{ r: 8 }}
                  dot={false}
                  name="Average Score of All Drivers"
                />
              </LineChart>
            ) : (
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Typography variant="subtitle1" sx={{ color: '#2C3E50', fontWeight: '500', ml: 5 }}>
              -------- Average Safety Score of all Drivers
            </Typography>
          </Box>
        </Box>
      </Box>
