...
import { getDriverEvents } from '../../services/fleetManager/driverServices';
import LoadingScreen from '../../components/molecules/loading-screen';
import { useSelector } from 'react-redux';

const DriverEventOverview = ({ driverId, driverLonestarId }: any) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await getDriverEvents(driverId, fromDate, toDate, driverLonestarId);
    ...
    fetchData();
  }, [dateRange, driverId, driverLonestarId]);
  ...
  const processData = (events: { tsInMilliSeconds: number; eventType: string }[]) => {
    ...
    const firstAvailable = dayjs(availableData[availableData.length - 1].tsInMilliSeconds).startOf('day');
            ...
            <ButtonGroup variant="contained" size="small">
              {Object.keys(eventTypeMap).map((key) => (
                <Button
                  ...
                  style={{ backgroundColor: colorMap[key], color: '#fff' }}
                >
                  {key}
                </Button>
              ))}
...
export default DriverEventOverview;
