...
const DevicesListTable: React.FC<any> = ({
  ...
  const columns = [
    { label: 'Organization ID', key: 'organizationId', minWidth: '110px', hideable: false },
    { label: 'Organization Name', key: 'organizationName', minWidth: '130px', hideable: true },
    { label: 'Insured ID', key: 'insuredId', minWidth: '90px', hideable: true },
    { label: 'Fleet Name', key: 'name', minWidth: '130px', hideable: true },
    { label: 'Fleet ID', key: 'lonestarId', minWidth: '90px', hideable: true },
    { label: 'Device Provider', key: 'partnerName', minWidth: '110px', hideable: true },
    { label: 'Device ID', key: 'deviceId', minWidth: '105px', hideable: false },
    { label: 'Device Status', key: 'status', minWidth: '90px', hideable: true },
    { label: 'IMEI Number', key: 'imei', minWidth: '115px', hideable: true },
    { label: 'Last Status Ping', key: 'lastStatusPing', minWidth: '130px', hideable: true },
    { label: 'Last Location Ping', key: 'lastLocationPing', minWidth: '130px', hideable: true },
                  ...
                  {columns
                    ...
                    .map(({ key, minWidth }, index, filteredArray) => {
                      ...
                      if (key === 'organizationName') {
                        return (
                          <StyledDataCell {...commonProps}>
                            {device?.lookup_FleetInsurers?.[0]?.name || 'NA'}
                          </StyledDataCell>
                        );
                      }
                      if (key === 'insuredId') {
                        return <StyledDataCell {...commonProps}>{device?.insuredId || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'name') {
                        return (
                          <StyledDataCell {...commonProps}>
                      ...
                      if (key === 'lonestarId') {
                        return <StyledDataCell {...commonProps}>{device?.lonestarId || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'partnerName') {
                        return <StyledDataCell {...commonProps}>{device?.deviceProvider || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'deviceId') {
                        return <StyledDataCell {...commonProps}>{device?.deviceId || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'status') {
                        return (
                          <StyledDataCell {...commonProps}>
                      ...
                      if (key === 'actions') {
                        return (
                          <StyledDataCell
                            {...commonProps}
                            sx={{ ...commonProps.sx, zIndex: 3, padding: '10px' }} // Higher z-index for actions to stay above other cells
                          >
