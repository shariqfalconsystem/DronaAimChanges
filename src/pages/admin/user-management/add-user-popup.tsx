...
interface OrgRoleMapping {
  ...
  userRole: string;
  roleUid?: string;
  isNew?: boolean; // Track if this is a new mapping
  isModified?: boolean; // Track if existing mapping was modified
}
...
const AddEditUserDialog: React.FC<AddUserDialogProps> = ({
  ...
  const resetForm = useCallback(() => {
    const defaultOrgType = organizationsTypes?.find((type) => type.enabled)?.orgType || '';
    reset({
      title: 'Mr.',
      firstName: '',
      lastName: '',
  ...
  useEffect(() => {
    if (mode === 'edit' && editUserData) {
      if (editUserData.orgRoleAndScoreMapping && editUserData.orgRoleAndScoreMapping.length > 0) {
        ...
        setOriginalMappings([initialMapping]);
  ...
  const removeMapping = (index: number) => {
    ...
    // If it's an existing mapping (has roleUid), add it to removedMappings for deletion
  ...
  const formatOrgRoleMappings = (mappings: any[], mode: string) => {
    ...
    mappings.forEach((mapping) => {
      ...
      if (mode === 'edit') {
        if (mapping.isNew) {
          ...
          action = 'update'; // Existing unchanged mapping
        }
      }
      ...
      // Include roleUid only for update actions
    ...
    // Handle removed mappings (only in edit mode)
    if (mode === 'edit') {
      removedMappings.forEach((removedMapping) => {
  ...
  const enabledOrgTypes = React.useMemo(() => {
    return (
      organizationsTypes?.filter((type) => {
        if (type.orgType === 'others') {
          return type.enabled && getOrganizationsByType('others').length > 0;
        }
        return type.enabled;
      }) || []
    );
  }, [organizationsTypes, getOrganizationsByType]);
  return (
    <>
      <Dialog
        open={open}
      ...
      >
        ...
        <DialogContent sx={{ p: 3 }}>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            {/* Dynamic Organization Role Mappings */}
                      ...
                      render={({ field }) => (
                        <FormControl fullWidth error={!!errors.orgRoleMappings?.[index]?.organization}>
                          <Select
                            {...field}
                            size="small"
                            disabled={!watchedMappings?.[index]?.organizationType}
                            sx={{
                              '& .MuiSelect-select': {
                                backgroundColor: !watchedMappings?.[index]?.organizationType
                                  ? '#D9E2EA'
                                  : 'transparent',
                              },
                            }}
