...
import { addFmUser, editFmUser } from '../../services/fleetManager/userMangtService';
...
const AddEditUserDialog: React.FC<AddUserDialogProps> = ({
  ...
  // Helper function to check if an organization is the current user's organization
  const isCurrentOrganization = (orgId: string) => {
    return orgId === lonestarId;
  };
  ...
  const filterCurrentOrgMappings = (mappings: any[]) => {
    return mappings.filter((roleMapping: any) => {
      // For fleetcompany roles, check if lonestarId matches current organization
      if (['fleetManager', 'fleetManagerSuperUser', 'driver'].includes(roleMapping.role)) {
        return roleMapping.lonestarId === lonestarId;
      }
      // For other roles, you might want to add additional logic based on your requirements
      // For now, we'll exclude them as they're not part of the current organization
      return false;
    });
  };
  ...
  useEffect(() => {
    if (mode === 'edit' && editUserData) {
      if (editUserData.orgRoleAndScoreMapping && editUserData.orgRoleAndScoreMapping.length > 0) {
        ...
        if (
          (editUserData.role === 'fleetManager' ||
            editUserData.role === 'fleetManagerSuperUser' ||
            editUserData.role === 'driver') &&
          editUserData.lonestarId === lonestarId
        ) {
          organizationValue = editUserData.lonestarId;
          organizationType = 'fleetcompany';
        }
  ...
  }, [mode, open, editUserData, reset, lonestarId]);
  ...
  const handleOrganizationTypeChange = (index: number, value: string) => {
    ...
    // Mark as modified if it's an existing mapping
  ...
  const handleDeleteConfirmation = () => {
    ...
    // Don't actually delete the last role - just close the popup
  };
  ...
  const formatOrgRoleMappings = (mappings: any[], mode: string) => {
    ...
    const currentOrgMappings = mappings.filter((mapping) => {
      return mapping.organization === lonestarId;
    });
    ...
    if (mode === 'edit') {
      ...
      const currentOrgRemovedMappings = removedMappings.filter((removedMapping) => {
        return removedMapping.organization === lonestarId;
      });
  ...
  const onSubmit = async (data: any) => {
    ...
    if (mode === 'add') {
      try {
        const { status, data: responseData } = await addFmUser(currentUserId, formattedData);
    ...
    if (mode === 'edit') {
      ...
      try {
        const { status, data: responseData } = await editFmUser(currentUserId, formattedData);
  ...
  const enabledOrgTypes =
    organizationsTypes?.filter((type: any) => type.enabled && type.orgType === 'fleetcompany') || [];
