                    ...
                    {columns
                      ...
                      .map(({ key, minWidth }, colIndex, filteredArray) => {
                        const commonProps = {
                          ...
                          sx: {
                            minWidth,
                            position: colIndex === 0 || key === 'actions' ? 'sticky' : 'static',
                            left: colIndex === 0 ? 0 : 'auto',
                            right: key === 'actions' ? 0 : 'auto',
                            zIndex: colIndex === 0 || key === 'actions' ? 2 : 'auto',
                            backdropFilter: applyBackdropFilter ? 'blur(50px)' : 'none',
                            background: colIndex === 0 || key === 'actions' ? (applyBackdropFilter ? '#f5f5f5' : '#fff') : 'inherit',
                            boxShadow:
                              colIndex === 0
                                ? '2px 0 5px rgba(0,0,0,0.1)'
                                : key === 'actions'
                        ...
                        if (key === 'lastLoginAtDisplay') {
                          return (
                            <StyledDataCell {...commonProps}>
                              {`${user.lastLoginAtDisplay || 'NA'}`}
                            </StyledDataCell>
                          );
                        }
                        if (key === 'emailId') {
                        ...
                        if (key === 'organization') {
                          return <StyledDataCell {...commonProps}>{renderOrganization(orgRoleMappings)}</StyledDataCell>;
                        }
                        if (key === 'role') {
                          return <StyledDataCell {...commonProps}>{renderRoles(orgRoleMappings, user)}</StyledDataCell>;
                        }
                        if (key === 'activeStatus') {
                          return (
                            <StyledDataCell {...commonProps}>
