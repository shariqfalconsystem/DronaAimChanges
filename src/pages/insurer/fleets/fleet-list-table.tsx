...
const FleetListTable: React.FC<any> = ({
  ...
  const columns = [
    { label: 'Insured ID', key: 'insuredId', minWidth: '120px', hideable: true },
    { label: 'Fleet ID', key: 'lonestarId', minWidth: '120px', hideable: true },
    { label: 'Fleet Name', key: 'name', minWidth: '150px', hideable: true },
    { label: 'Organization ID', key: 'insurerId', minWidth: '150px', hideable: true },
    { label: 'Organization Name', key: 'organizationName', minWidth: '180px', hideable: true },
    { label: 'DOT No.', key: 'dotNumber', minWidth: '120px', hideable: true },
    { label: 'Fleet Score', key: 'meanScore', minWidth: '120px', hideable: true },
    { label: 'Miles - Last 30 Days', key: 'lastThirtyDayTotalDistanceInMiles', minWidth: '150px', hideable: true },
    { label: 'Primary Contact', key: 'primaryContactFullName', minWidth: '180px', hideable: true },
    { label: 'Phone', key: 'primaryContactPhone', minWidth: '130px', hideable: true },
    { label: 'Email', key: 'primaryContactEmail', minWidth: '200px', hideable: true },
    { label: 'Policy Number', key: 'policyNumber', minWidth: '150px', hideable: true },
    { label: ' Policy Status', key: 'policyStatus', minWidth: '120px', hideable: true },
    { label: 'Policy Effective Date', key: 'termEffectiveDate', minWidth: '150px', hideable: true },
    { label: 'Policy Expiry Date', key: 'termExpiryDate', minWidth: '150px', hideable: true },
    { label: 'Action', key: 'actions', minWidth: '150px', hideable: false },
  ];
                    ...
                    >
                      {columns
                        ...
                        .map(({ key }) => {
                          ...
                          if (key === 'lonestarId') {
                            return <StyledDataCell key={key}>{fleet?.lonestarId || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'name') {
                            return (
                          ...
                          if (key === 'insurerId') {
                            return <StyledDataCell key={key}>{fleet?.lookup_FleetInsurers?.[0]?.insurerId || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'organizationName') {
                            return <StyledDataCell key={key}>{fleet?.lookup_FleetInsurers?.[0]?.name || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'dotNumber') {
                            return <StyledDataCell key={key}>{fleet?.dotNumber || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'meanScore') {
                            return (
                          ...
                          if (key === 'primaryContactPhone') {
                            return <StyledDataCell key={key}>{fleet?.primaryContactPhone || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'primaryContactEmail') {
                            return <StyledDataCell key={key}>{fleet?.primaryContactEmail || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'policyNumber') {
                            return <StyledDataCell key={key}>{fleet?.lookup_Policies?.[0]?.policyId || 'NA'}</StyledDataCell>;
                          }
                          if (key === 'policyStatus') {
                            return <StyledDataCell key={key}>{<ChipWithIcon status={formattedStatus} />}</StyledDataCell>;
                          }
