...
const HelpCenter = () => {
  ...
  const handleTopicClick = async (item: Topic) => {
    try {
      navigate(`/insurer/help-center/view-content/${item.publishedId}`);
    } catch (error) {
  ...
  const handleSharePDF = async (topic: Topic) => {
    try {
      ...
      const viewContentUrl = `${window.location.origin}/insurer/help-center/view-content/${topic.publishedId}`;
