...
const UnassignedListTable: React.FC<any> = ({
  ...
  const columns = [
    ...
    { label: 'Fleet Name', key: 'fleetName', minWidth: '130px', hideable: true },
    ...
    { label: 'Shipping Provider', key: 'shippingProviderName', minWidth: '120px', hideable: true },
    { label: 'Tracking Number', key: 'trackingNumber', minWidth: '120px', hideable: true },
    { label: 'Date of Shipment', key: 'shipmentDate', minWidth: '110px', hideable: true },
    { label: 'Device Snapshots', key: 'snapshot', minWidth: '110px', hideable: true },
                  ...
                  {columns
                    ...
                    .map(({ key }) => {
                      ...
                      if (key === 'shippingProviderName') {
                        return <StyledDataCell key={key}>{device?.shippingProviderName || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'trackingNumber') {
                        return <StyledDataCell key={key}>{device?.trackingNumber || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'shipmentDate') {
                        return <StyledDataCell key={key}>{device?.shipmentDate || 'NA'}</StyledDataCell>;
                      }
                      if (key === 'snapshot') {
                        return (
                          <StyledDataCell key={key}>
