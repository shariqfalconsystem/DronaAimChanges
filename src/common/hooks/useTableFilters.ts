...
function useTableFilter<T>({
  ...
  const filteredData = useMemo(() => {
    ...
    return localData?.filter((item: any) => {
      ...
      const isScoreInRange = filterCriteria?.scoreRange
        ? item.tripScore == null ||
          (item.tripScore >= filterCriteria?.scoreRange[0] && item?.tripScore <= filterCriteria?.scoreRange[1])
        : true;
      ...
      return matchesSearch && (hasNoCriteria || (isDateInRange && isStatusMatched && isScoreInRange));
    });
  ...
  const sortedData = useMemo(() => {
    ...
    sorted.sort((a: any, b: any) => {
      ...
      if (!sortColumn) {
        const aIsInProgress = a.tripStatus === 'Started';
        const bIsInProgress = b.tripStatus === 'Started';

        // Check if they are orphaned
        const aIsOrphaned = a.isOrphaned;
        const bIsOrphaned = b.isOrphaned;

        if (aIsInProgress && !aIsOrphaned && (!bIsInProgress || bIsOrphaned)) return -1;
        if (bIsInProgress && !bIsOrphaned && (!aIsInProgress || aIsOrphaned)) return 1;
      }
      ...
      if (sortColumn) {
        ...
        if (sortColumn === 'formattedDistance') {
          aValue = parseFloat(a.tripDistance);
          bValue = parseFloat(b.tripDistance);
        }
